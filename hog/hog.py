...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max(score0, score1) < goal:
        if who == 0:
            player_0_rolls = strategy0(score0, score1)
            current0_points = take_turn(player_0_rolls, score1, dice)
            score0 += current0_points
            if abs(player_0_rolls-previous0_points)==2 and feral_hogs:
                score0 += 3
            previous0_points = current0_points
            if is_swap(score0, score1):
                score0, score1 = score1, score0
        else:
            player_1_rolls = strategy1(score1, score0)
            current1_points = take_turn(player_1_rolls, score0, dice)
            score1 += current1_points
            if abs(player_1_rolls-previous1_points)==2 and feral_hogs:
                score1 += 3
            previous1_points = current1_points
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    expected_pts = free_bacon(opponent_score)
    if expected_pts >= cutoff:
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    pts_0_roll = free_bacon(opponent_score)
    score_with_pts_0_roll = score + pts_0_roll
    if is_swap(score_with_pts_0_roll, opponent_score):
        if score_with_pts_0_roll < opponent_score:
            return 0
        return num_rolls
    if pts_0_roll >= cutoff:
        return 0
    return num_rolls
